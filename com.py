...
with a1:
    ...
    bar_chart = (
        alt.Chart(source)
        .mark_bar()
        .encode(x="sum(Quantity ($)):Q", y=alt.Y("Produit:N", sort="-x"))
    )
...
with b2:
    ...
    bar_chart = (
        ...
        .mark_bar()
        .encode(x="month(Date):O", y="sum(UnitPrice ($)):Q", color="Produit:N")
    )
...
with c2:
    ...
    ax.set_title(f"Histogramme of {feature}")
